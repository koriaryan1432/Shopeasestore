...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (name, email, password, phone_number) => {
    const { data } = await api.post('/auth/register', { name, email, password, phone_number });
    localStorage.setItem('token', data.token);
    localStorage.setItem('user', JSON.stringify(data.user));
    setUser(data.user);
    return data.user;
  ...
  const verifyOTP = async (identifier, code, type) => {
    const { data } = await api.post('/auth/otp/verify', { identifier, code, type });
  ...
  return (
    <AuthContext.Provider value={{ user, login, register, loginWithGoogle, sendOTP, verifyOTP, logout }}>
      {children}
